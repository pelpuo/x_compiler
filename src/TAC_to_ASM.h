...
private:
  ...
  std::map<std::string, std::string>
      floatConstantLabels; // Maps float literal to its .rodata label

  std::vector<std::pair<std::string, std::string>> pendingArgs;

  // --- Type and Register Helpers ---

  bool isFloatType(const std::string &typeStr) { return typeStr == "double"; }
  ...
  std::string ensureLoaded(const std::string &name) {
    ...
    if (varMap.count(name)) {
      ...
      std::string op =
          (info.type == "int" || info.type == "unsigned_int") ? "lw" : "ld";
    ...
    for (const auto &tac : dataTACs) {
      if (tac.op == "StaticVariable") {
        ...
        // --- MODIFY THIS LINE ---
          ...
          // Now handles long, ulong, AND any pointer type
          outfile << "    .quad " << tac.result << "\n";
        } else if (isFloatType(type)) {
          outfile << "    .double " << tac.result << "\n";
        }
      }
    }
  }
...
public:
  ...
  void generateAssembly(const std::vector<TAC> &tacCode) {
    ...
    for (const auto &tac : codeTACs) {
      // outfile << "# " << tac.op << " " << tac.result << ", " << tac.arg1 <<
      // ", " << tac.arg2 << "\n";
      ...
      // Memory Operations (Store/Load)
        ...
        const std::string &type = tac.arg2;
        // --- THIS 'IF' BLOCK IS FOR LOCALS (Correct) ---
        if (varMap.count(tac.result)) {
          if (isFloatType(type)) {
            outfile << "    fsd " << ensureFloatLoaded(tac.arg1) << ", "
                    << varMap[tac.result].offset << "(s0)\n";
          } else {
            std::string op =
                (type == "int" || type == "unsigned_int") ? "sw" : "sd";
            outfile << "    " << op << " " << ensureLoaded(tac.arg1) << ", "
                    << varMap[tac.result].offset << "(s0)\n";
          }
        }
        // --- ADD THIS 'ELSE' BLOCK FOR GLOBALS ---
        else {
          // Handle global variables
          std::string addrReg = getTempReg(); // Use a scratch register like t6
          outfile << "    la " << addrReg << ", " << tac.result
                  << "\n"; // Load address of global

          if (isFloatType(type)) {
            std::string srcReg = ensureFloatLoaded(tac.arg1);
            outfile << "    fsd " << srcReg << ", 0(" << addrReg << ")\n";
          } else {
            std::string srcReg = ensureLoaded(tac.arg1);
            std::string op =
                (type == "int" || type == "unsigned_int") ? "sw" : "sd";
            outfile << "    " << op << " " << srcReg << ", 0(" << addrReg
                    << ")\n";
          }
        }
      } else if (tac.op == "load") {
        ...
        if (isFloatType(type)) {
          ...
          if (varMap.count(tac.arg1))
            outfile << "    fld " << dstReg << ", " << varMap[tac.arg1].offset
                    << "(s0)\n";
          else {
          ...
          std::string op =
              (type == "int" || type == "unsigned_int") ? "lw" : "ld";
          if (varMap.count(tac.arg1))
            outfile << "    " << op << " " << dstReg << ", "
      ...
      // Load Immediate
        ...
        if (isFloatType(tac.arg2)) {
          ...
          outfile << "    fld " << mapToFloatRegister(tac.result)
                  << ", 0(t6)\n";
        } else {
      ...
      else if (tac.op == "+" || tac.op == "-" || tac.op == "*" ||
               tac.op == "/") {
        if (floatRegisterMap.count(tac.arg1) ||
            floatRegisterMap.count(tac.arg2)) {
          std::map<std::string, std::string> opMap = {{"+", "fadd.d"},
                                                      {"-", "fsub.d"},
                                                      {"*", "fmul.d"},
                                                      {"/", "fdiv.d"}};
          outfile << "    " << opMap[tac.op] << " "
                  << mapToFloatRegister(tac.result) << ", "
                  << ensureFloatLoaded(tac.arg1) << ", "
                  << ensureFloatLoaded(tac.arg2) << "\n";
        } else {
          std::map<std::string, std::string> opMap = {
              {"+", "add"}, {"-", "sub"}, {"*", "mul"}, {"/", "div"}};
          if (isImmediate(tac.arg2) && tac.op == "+")
            outfile << "    addi " << mapToRegister(tac.result) << ", "
                    << ensureLoaded(tac.arg1) << ", " << tac.arg2 << "\n";
          else
            outfile << "    " << opMap[tac.op] << " "
                    << mapToRegister(tac.result) << ", "
                    << ensureLoaded(tac.arg1) << ", " << ensureLoaded(tac.arg2)
                    << "\n";
        }
      }
      // Integer-only Bitwise and Remainder
        ...
        std::map<std::string, std::string> opMap = {
            {"%", "rem"}, {"&", "and"},  {"|", "or"},
            {"^", "xor"}, {"<<", "sll"}, {">>", "sra"}};
        outfile << "    " << opMap[tac.op] << " " << mapToRegister(tac.result)
                << ", " << ensureLoaded(tac.arg1) << ", "
                << ensureLoaded(tac.arg2) << "\n";
      }
      // Comparison Operations
        ...
        if (floatRegisterMap.count(tac.arg1) ||
          ...
          if (tac.op == "==")
            ...
            outfile << "    flt.d " << dst << ", " << freg1 << ", " << freg2
                    << "\n";
          else if (tac.op == "<=")
            outfile << "    fle.d " << dst << ", " << freg1 << ", " << freg2
                    << "\n";
          else if (tac.op == ">")
            outfile << "    flt.d " << dst << ", " << freg2 << ", " << freg1
                    << "\n";
          else if (tac.op == ">=")
            outfile << "    fle.d " << dst << ", " << freg2 << ", " << freg1
                    << "\n";
        } else {
          std::string dst = mapToRegister(tac.result);
          ...
          if (tac.op == "==") {
            ...
            outfile << "    slt " << dst << ", " << reg1 << ", " << reg2
                    << "\n";
          else if (tac.op == ">")
            outfile << "    slt " << dst << ", " << reg2 << ", " << reg1
                    << "\n";
          else if (tac.op == "<=") {
            outfile << "    slt " << dst << ", " << reg2 << ", " << reg1
                    << "\n";
      ...
      else if (tac.op == "move" || tac.op == "Copy")
      ...
      // Conditional Branches
        ...
        outfile << "    beqz " << ensureLoaded(tac.arg1) << ", " << tac.arg2
                << "\n";
      else if (tac.op == "bnez")
        outfile << "    bnez " << ensureLoaded(tac.arg1) << ", " << tac.arg2
                << "\n";
      else if (tac.op == "beq")
        outfile << "    beq " << ensureLoaded(tac.arg1) << ", "
                << ensureLoaded(tac.arg2) << ", " << tac.result << "\n";
      else if (tac.op == "bne")
        outfile << "    bne " << ensureLoaded(tac.arg1) << ", "
                << ensureLoaded(tac.arg2) << ", " << tac.result << "\n";
      else if (tac.op == "blt")
        outfile << "    blt " << ensureLoaded(tac.arg1) << ", "
                << ensureLoaded(tac.arg2) << ", " << tac.result << "\n";
      else if (tac.op == "bgt")
        outfile << "    bgt " << ensureLoaded(tac.arg1) << ", "
                << ensureLoaded(tac.arg2) << ", " << tac.result << "\n";
      else if (tac.op == "ble")
        outfile << "    ble " << ensureLoaded(tac.arg1) << ", "
                << ensureLoaded(tac.arg2) << ", " << tac.result << "\n";
      else if (tac.op == "bge")
        outfile << "    bge " << ensureLoaded(tac.arg1) << ", "
                << ensureLoaded(tac.arg2) << ", " << tac.result << "\n";
      // Function Call Machinery
      else if (tac.op == "arg")
        pendingArgs.push_back({tac.arg1, tac.arg2});
      else if (tac.op == "call") {
        int currentIntArg = 0, currentFloatArg = 0;
        for (const auto &arg : pendingArgs) {
          if (isFloatType(arg.second)) {
            if (currentFloatArg < 8)
              outfile << "    fmv.d fa" << currentFloatArg++ << ", "
                      << ensureFloatLoaded(arg.first) << "\n";
          } else {
            if (currentIntArg < 8)
              outfile << "    mv a" << currentIntArg++ << ", "
                      << ensureLoaded(arg.first) << "\n";
          }
        }
        pendingArgs.clear();
        ...
        if (!tac.result.empty()) {
          if (isFloatType(tac.arg2))
            outfile << "    fmv.d " << mapToFloatRegister(tac.result)
                    << ", fa0\n";
          else
        ...
        if (varMap.find(tac.arg1) == varMap.end()) {
          int size = isFloatType(type)
                         ? 8
                         : (type == "int" || type == "unsigned_int" ? 4 : 8);
          stackOffset -= size;
          varMap[tac.arg1] = {stackOffset, type};
        }
        if (isFloatType(type))
          outfile << "    fsd fa" << floatParamIndex++ << ", "
                  << varMap[tac.arg1].offset << "(s0)\n";
        else {
          std::string op =
              (type == "int" || type == "unsigned_int") ? "sw" : "sd";
          outfile << "    " << op << " a" << intParamIndex++ << ", "
                  << varMap[tac.arg1].offset << "(s0)\n";
        }
      }
      // Casting and Type Conversion
      else if (tac.op == "IntToDouble")
        outfile << "    fcvt.d.w " << mapToFloatRegister(tac.result) << ", "
                << ensureLoaded(tac.arg1) << "\n";
      else if (tac.op == "DoubleToInt")
        outfile << "    fcvt.w.d " << mapToRegister(tac.result) << ", "
                << ensureFloatLoaded(tac.arg1) << ", rtz\n";
      else if (tac.op == "SignExtend")
        outfile << "    sext.w " << mapToRegister(tac.result) << ", "
                << ensureLoaded(tac.arg1) << "\n";
      else if (tac.op == "Truncate")
        outfile << "    addiw " << mapToRegister(tac.result) << ", "
                << ensureLoaded(tac.arg1) << ", 0\n";
        ...
        std::string dstReg = mapToRegister(tac.result);
        if (varMap.count(tac.arg1)) {
          outfile << "    addi " << dstReg << ", s0, "
                  << varMap[tac.arg1].offset << "\n";
        } else {
          // --- ADD THIS BLOCK ---
          // Handle global variables
        ...
        const std::string &type = tac.arg2;
        std::string srcPtrReg =
            ensureLoaded(tac.arg1); // Register holding the address
        ...
        const std::string &type = tac.arg2;
        std::string dstPtrReg =
            ensureLoaded(tac.result); // Register holding the address

        if (isFloatType(type)) {
          std::string srcValReg = ensureFloatLoaded(tac.arg1);
          outfile << "    fsd " << srcValReg << ", 0(" << dstPtrReg << ")\n";
        } else {
          std::string srcValReg =
              ensureLoaded(tac.arg1); // Register with the value
          std::string op =
              (type == "int" || type == "unsigned_int") ? "sw" : "sd";
      ...
      // Unhandled
        ...
        std::cerr << "FATAL: Unknown TAC operation in ASM generation: "
                  << tac.op << "\n";
        exit(1);
      }
    }
